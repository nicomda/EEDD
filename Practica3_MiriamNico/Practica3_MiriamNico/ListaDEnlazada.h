...
public:
	...
	ListaDEnlazada() { cabecera = cola = 0; }
	...
	//creamos los metodos de borrar los datos al inicio y al final, y en una posici�n dada.
	void borrarInicio();
	void borrarFinal();
...
void ListaDEnlazada<T>::insertar(Iterador<T>& i, T& mdato) {
	...
	if (i.getNodo() == cabecera) {
		...
		if (i.getNodo() == cola) insertarFinal(mdato);
			...
			Nodo<T>* nodo = new Nodo<T>(mdato, i.getNodo()->getAnterior(), i.getNodo());
